...
def post_rest_call(test, url, params={}, post_header={}, json=None, expected_code=200):
    """Implements a REST api using the POST verb"""
    response = requests.post(url, params=params, headers=post_header, json=json)
    test.assertEqual(
        expected_code,
        response.status_code,
        f"Response code to {url} not {expected_code}",
...
def put_rest_call(test, url, params={}, put_header={}, json=None, expected_code=200):
    """Implements a REST api using the PUT verb"""
    response = requests.put(url, params=params, headers=put_header, json=json)
    test.assertEqual(
        expected_code,
        response.status_code,
        f"Response code to {url} not {expected_code}",
...
def delete_rest_call(test, url, delete_header={}, json=None, expected_code=200):
